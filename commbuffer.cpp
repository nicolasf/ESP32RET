...
uint32_t CommBuffer::Crc32(const uint8_t* data, int start, int numBytes)
{
    ...
    for (size_t i = start; i < numBytes + start; i++) {
...
{
	int i;
  int start_transmitBufferLength = transmitBufferLength;
	uint32_t crc = 0;

    transmitBuffer[transmitBufferLength++] = 0x66;
	...
	int frame_len = transmitBufferLength - start_transmitBufferLength;

	if(frame.length < 8)
	{
		for(i = frame_len; i < 16; i++)
		{
			transmitBuffer[transmitBufferLength++] = 0x00;
		}
	}

    crc = Crc32(transmitBuffer, start_transmitBufferLength, frame_len);
	transmitBuffer[transmitBufferLength++] = (crc & 0xFF);
	transmitBuffer[transmitBufferLength++] = (crc & 0x0000FF00)>>8;
	transmitBuffer[transmitBufferLength++] = (crc & 0x00FF0000)>>16;
	transmitBuffer[transmitBufferLength++] = (crc & 0xFF000000)>>24;
}    
